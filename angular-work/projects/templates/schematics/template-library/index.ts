...
export default function (options:any): Rule {
  return async (host: Tree) => {
    ...
    const projectRoot = `projects/${packageName}/src/public-api.ts`
    const tsConfigPackageName = `@windmillcode/${packageName}`
    return chain([
      mergeWith(templateSource),
      addLibToWorkspaceFile(options, libDir, packageName),
      options.skipPackageJson ? noop() : addDependenciesToPackageJson(),
      options.skipTsConfig ? noop() : updateTsConfig(tsConfigPackageName, projectRoot),
      schematic('module', {
        name: options.name,
        commonModule: false,
        flat: true,
